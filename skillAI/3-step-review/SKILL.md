...
| | |
|---|---|
| File | **75 file điều tra md + 43 sheet phán định (7 `batch_decision.xlsx`) + `summary_batch_migration_ja.md`** trong `submit_folder/2026_08_13/`. Đây là tài liệu **của member**, không phải của SYP-AI → áp nguyên tắc #7 mục 1 (chỉ review TÍNH CHÍNH XÁC, miễn TEMPLATE v4, giữ phong cách member). Plan: `review_plan_20260813.md`; kết quả tích lũy: `review_summary.md`; bản sửa: `new/` trong từng thư mục nhóm (file gốc không đụng, xlsx KHÔNG BAO GIỜ sửa). **`new/` là BỘ TÀI LIỆU NỘP nên phải ĐẦY ĐỦ** (user chốt 17/08): file có finding = bản đã sửa, file không finding = **bản sao nguyên văn** — thiếu file thì người nhận không phân biệt được "không đổi" với "bỏ sót". **Mặt phẳng copy-paste cho member = `new/batch_decision.md`, có ở CẢ 7 nhóm có xlsx** (quy ước user chốt 17/08, thay quy ước cũ "chỉ nhóm có 要修正"): bản **修正版** cho nhóm có sheet 要修正 (G1/G2/G4 — thay câu trong ô; G3 — sheet chỉ trỏ tên file nên thêm dòng 「レビュー結果」 ngoài xlsx, member sửa FILE md chứ không sửa ô) và bản **レビュー確認版** trùng khít bản gốc cho nhóm 0 sheet 要修正 (G5/G6/G7). Meta đầu mỗi file ghi verdict từng sheet + trỏ QA-0x |
...
| Điểm kiểm đặc thù nếu review lại | ① **Quy ước dịch JA đã chốt**: văn thể **である体**(0/24 file `_ja` member dùng ます/です), code thật giữ nguyên 100% kể cả comment VN, sơ đồ ASCII văn xuôi thì dịch; heading thống nhất 概要・第A部/第B部 ―・対照・確認済み・まとめ. ② Sheet **妥当だが根拠不足 KHÔNG thay câu trong bảng** (chỉ sheet 要修正) — **6 sheet** có văn bản JP soạn sẵn, cố ý để ngoài bảng ở mục 付録 cuối `new/batch_decision.md` nhóm tương ứng với nhãn 【提案・未適用】: câu THAY THẾ cả ô (G1 `CalcYearlyRoomTemperature` ・ G2 `CreateGroupSummary` ・ G2 `RankingCreation` ・ G5 `SendAlertLogMail`) và câu NỐI THÊM sau câu hiện tại (G1 `CalcDailyAverageData` ・ G1 `CalcWeeklySavingReportEffect`) — xem `review_summary.md` mục 7-A2, đừng báo là sót. 13 sheet 根拠不足 còn lại không có văn bản soạn sẵn. ③ Lỗi **pre-existing** của member đã liệt kê ở mục 7-B (bảng vỡ cột do `\|` trong regex, đường dẫn `e:/Projects/...`, `docs/legacy-batch-review/...`) — không tính là lỗi đợt vá. ④ Kết luận về hệ mới bám pin `dc39aa39`: nhánh `feat/kitagas-batch-import-error-notification` **chưa merge**. ⑤ Cơ chế ước lượng token trước mỗi khối: `notes/usage_budget.md` (⛔#15) — đơn giá thật đã hiệu chỉnh: verifier ~120k ・ fixer ~65k/batch ・ **dịch ~120k/file** ・ checker ~144k |
