...
| | |
|---|---|
| File | **75 file điều tra md + 43 sheet phán định (7 `batch_decision.xlsx`) + `summary_batch_migration_ja.md`** trong `submit_folder/2026_08_13/`. Đây là tài liệu **của member**, không phải của SYP-AI → áp nguyên tắc #7 mục 1 (chỉ review TÍNH CHÍNH XÁC, miễn TEMPLATE v4, giữ phong cách member). Plan: `review_plan_20260813.md`; kết quả tích lũy: `review_summary.md`; bản sửa: `new/` trong từng thư mục nhóm (file gốc không đụng, xlsx KHÔNG BAO GIỜ sửa). **Mặt phẳng copy-paste cho member = `new/batch_decision.md`, có ở CẢ 7 nhóm có xlsx** (quy ước user chốt 17/08, thay quy ước cũ "chỉ nhóm có 要修正"): bản **修正版** cho nhóm có sheet 要修正 (G1/G2/G4 — thay câu trong ô; G3 — sheet chỉ trỏ tên file nên thêm dòng 「レビュー結果」 ngoài xlsx, member sửa FILE md chứ không sửa ô) và bản **レビュー確認版** trùng khít bản gốc cho nhóm 0 sheet 要修正 (G5/G6/G7). Meta đầu mỗi file ghi verdict từng sheet + trỏ QA-0x |
