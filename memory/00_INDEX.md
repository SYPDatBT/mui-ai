...
- **ステータス của QAデータベース có BA giá trị**: `回答中` (chưa dùng làm căn cứ được) / `回答済` / **`完了`** (≡ 回答済, user xác nhận). Guide trước chỉ liệt kê 2 → **grep `回答済` là sót phiếu đã đóng**. Kèm 2 bẫy mới đã ghi vào Phụ lục E.2: **質問内容 có thể Empty** (nội dung nằm ở body trang) ・ **更新日時 hiện kiểu tương đối** ("Last Thursday"), phải lấy ngày tuyệt đối trước khi trích.
...
1c. **RẺ MÀ ĐÁNG LÀM NGAY — mở lại QAデータベース kiểm trạng thái**: 4 QA hồi 08-03/04 mà guide §9.4 vẫn ghi 「回答中」 **có thể đã 完了 hết** (🔸 chưa kiểm); cộng 5 trang 回答中 ở việc số 8. Một lượt rà là hạ được nhiều nhãn 🔸 trong guide. Nhớ: `完了` cũng là "đã trả lời" — **grep `回答済` sẽ sót**.
...
| ⭐ `09_session_20260820_qaSypScopeNo10_guideBangTanto.md` | [2026-08-20] **Phạm vi SYP CHỐT** qua QA Notion 「SYP開発範囲の確認」 No. 10 (回答者 swan, 08-13, **完了**): mui Lab = 7-1 ファームウェア + 7-2 GW管理クラウド │ SYP = 7-3 EMINEL-smartサーバー + 7-4 管理画面 + **モバイルアプリ** → guide vá **6 chỗ, +93/−13** (§1.6 tách 対象範囲 vs **担当** ・ §6.1 bảng mã thêm cột 担当 ・ §9.4 hạ nhãn 🔸 ・ Phụ lục E.2) ・ **ステータス có 3 giá trị, `完了` bị thiếu** trong guide ・ **lỗ hổng: đợt v1.3 chỉ rà git, không rà Notion** ・ bài học: checker anchor tự viết báo 62 link hỏng = SAI (họ ⛔#13) |
