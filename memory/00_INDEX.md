...
1b. **Đuôi của đợt vá 08-20**: ① ✅ review vùng sửa XONG 21/08 sáng (commit `89c6766`, vá 4 finding) ② **CHƯA quyết** guide có lên **v1.4** không — bảng meta đầu guide vẫn ghi `1.3` ③ ✅ commit + push đủ (21/08 chiều: **AI đã được cấp quyền `git push`** qua `.claude/settings.local.json` — user ra lệnh văn xuôi "push đi/cho lên github" là AI push thẳng; ⛔#8 vẫn giữ: không push khi user CHƯA yêu cầu) ④ **CHƯA rà** `requirements/README.md` + `notes/guide_v13_mapping.md` theo bảng 担当 mới (`self_study_plan.md` dòng 54 vẫn còn câu sai — xem việc 4).
