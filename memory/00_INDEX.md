...
**[08-18] ⚠️ ĐỀ BÀI MỚI TỪ MUI — `submit_folder/2026_08_18/chokkin_irai.md`** (user tự đặt vào, AI **chưa phân tích**): dựng **môi trường dev riêng cho E-GW (`dev’`)** ・ **chiến lược branch** (E-GW: `syp-gw-dev` phái sinh từ `syp-dev` + `gw-develop`; E-Smart giữ `syp-dev`/`develop`/`main`) cho cả BE/admin/app ・ **tái cấu trúc mobile app** (`apps/{e-smart-app,e-gw-app}` + `package/{theme,ui_components}`) ・ **refactor E-Smart** sau khi có môi trường ・ **điều tra toàn bộ batch legacy** (đúng hạng mục đang làm dở). ⇒ **Cần bàn với user để xếp lại thứ tự ưu tiên bên dưới.**
