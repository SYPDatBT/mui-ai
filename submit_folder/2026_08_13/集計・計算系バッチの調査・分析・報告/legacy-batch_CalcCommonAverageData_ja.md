...
| **入力** | コマンドライン引数を自ら受け取ることはない ― 呼び出し元3バッチのいずれかから `executeCommon($type, $dateTime, $aggregationUnit)` 関数呼び出しを通じてのみ値を受け取る。DBの読み取りのみ：単位に対応する世帯別数値テーブル（`s_102`/`s_103`/`s_104`）＋グループ集計先テーブル（`s_112`/`s_113`/`s_114`。「グループ母数」の参照およびデータ欠損期間の検索にも再利用する）。 |
...
出典: `CalcCommonAverageDataCommand.php:112-407`（`retroactiveDaily/Monthly/Yearly` の3関数）、`418-815`（`updateRetroactiveData`、`getRetroactiveUpdatePeriod`、`getRetroactiveData`、`getAggregationTarget`、`getTargetGroup`）。
