...
| パラメータ | 値 | 意味 |
|---|---|---|
| `:calcDate` | `<yearmonth>-01 00:00:00` | `s_103`内で読み取るべき月行のキー（`CalcMonthlyAverageSetTemperatureCommand`が書き込む方式と一致：`datetime` = 計算対象日を含む月の01日） |
| `:calcYear` | `yearmonth`の先頭4文字 | 年 ― `s_104`へ書き込む際の年行のキーとして使用 |
| `:summaryThreshould` | 定数`NOT_SUMMARY_DATE_COUNT` = 10 | 当月内で許容されるデータ欠損日数の上限 |
...
s_103  "ConSensorDailyValue"    1行/世帯/月  × 31個の日カラム   (device_type=17, room_id=0)
   ...
   │   データが存在する日数 ≥ (当月の日数 − NOT_SUMMARY_DATE_COUNT=10) の場合のみ計算する)
   ▼
