...
Công thức dùng 4 thuộc tính (EPC) cùng lúc để quy đổi giá trị đọc thô sang kWh thực:
...
Không có bước chặn giá trị âm trong công thức này cho `GAS_POWER`/`SOLAR_GENERATION`/`BATTERY_DISCHARGE`/`BATTERY_CHARGE`/`SALE_ELECTRIC`/`BUY_ELECTRIC` — hiệu số âm được tính và ghi thẳng vào `s_102` như một giá trị bình thường. Với `DETECT_LIVING`/`DETECT_OTHER`, có bước kiểm tra riêng: loại các giá trị đặc biệt (cảm biến chưa gắn/không phản hồi) và giá trị ngoài dải hợp lệ (`0` ~ `999999999`) thành `null`; bước này không kiểm tra dấu của hiệu số.
...
Một batch khác cùng họ — `CalcDailyEnergyConsumptionCommand` — đọc 6 cột giờ mà batch này ghi vào `s_102` (gas phát điện, mặt trời, xả pin, sạc pin, và một phần bán điện) làm input để ráp công thức tổng tiêu thụ điện (`消費電力量`). Chi tiết công thức và batch phụ thuộc thứ hai (`RcvHalfHourElectricPowerCommand`, nguồn của 買電/một phần 売電) nằm ở tài liệu `legacy-batch_CalcDailyEnergyConsumption.md` trong cùng bộ tài liệu này.
