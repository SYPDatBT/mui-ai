...
| **入力** | DBの読み取りのみ、**外部APIは呼び出さず、CSVファイルも読み込まない**：`s_102`（**各世帯**の時間別の値。別バッチによって事前に算出済みで、`t_101`から非正規化された5つのグループ属性が付随する）＋ `s_113`（「グループの母数」――各グループに属する世帯数を該当時点で保持し、データ充足率のしきい値判定に用いるテーブル）＋ コマンドライン引数 `--type`, `--datetime`。 |
...
| 項目 | 内容 | 出典 |
|---|---|---|
| 実行スケジュール（cron） | `55 * * * *` ― 毎時、4種類まとめて1回実行（`type2_3_5_6`） | `docs/02_詳細設計/10_バッチ処理/mng-webap_cron設定_20241029.txt:21-22`（`#7.日毎平均データ算出` → `7_CalcDailyAverageData_type2_3_5_6.sh`） |
...
- 各グループについて、1時間ずつ遡り（最大192時間≈7日間、`retroactiveLimit`）、データが不足している時間帯を探す。
