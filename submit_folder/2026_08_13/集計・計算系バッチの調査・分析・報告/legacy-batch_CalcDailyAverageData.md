...
| **Input** | Chỉ đọc DB, **không gọi API ngoài, không đọc file CSV**: `s_102`（giá trị theo từng giờ của **từng hộ**, đã được batch khác tính sẵn, có kèm 5 thuộc tính nhóm denormalize từ `t_101`）＋ `s_113`（bảng chứa "mẫu số nhóm" — số hộ thuộc mỗi nhóm tại thời điểm tương ứng, dùng để kiểm tra ngưỡng đủ dữ liệu）＋ tham số dòng lệnh `--type`, `--datetime`. |
...
| Mục | Nội dung | Nguồn |
|---|---|---|
| Lịch chạy (cron) | `55 * * * *` — mỗi giờ, chạy 1 lần cho cả 4 loại (`type2_3_5_6`) | `docs/02_詳細設計/10_バッチ処理/mng-webap_cron設定_20241029.txt:21-22` (`#7.日毎平均データ算出` → `7_CalcDailyAverageData_type2_3_5_6.sh`) |
...
- Với mỗi nhóm, lùi dần từng giờ (tối đa 192 giờ ≈ 7 ngày, `retroactiveLimit`), tìm khung giờ còn thiếu dữ liệu.
