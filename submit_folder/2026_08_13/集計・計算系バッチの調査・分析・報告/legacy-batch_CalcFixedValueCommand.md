...
| **Output** | **Chỉ ghi DB** — với mỗi hộ, mỗi ngày có dữ liệu trả về, ghi 1 bản ghi (INSERT mới) vào `s_103`（entity `ConSensorDailyValue`, qua thư viện chung `EminelSvLib`）: set `device_type = BUY_ELECTRIC(11)`, 1 cột-ngày tương ứng (`c011`〜`c041`) + 5 cột thuộc tính nhóm. Không gửi mail, không xuất CSV. |
...
- Gọi `save()` để ghi entity mới xuống `s_103` (thao tác **insert bản ghi mới**, không đọc bản ghi cũ theo khóa chính lên để cập nhật trước khi ghi).
