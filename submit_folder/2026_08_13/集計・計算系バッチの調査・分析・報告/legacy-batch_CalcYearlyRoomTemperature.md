...
| Mục | Nội dung | Nguồn |
|---|---|---|
| Lịch chạy (cron) | `20 5 1 * * *` — 1 lần/tháng, ngày 1 lúc 05:20, chạy **quanh năm** (không giới hạn theo mùa) | `docs/02_詳細設計/10_バッチ処理/mng-webap_cron設定_20241029.txt:52` (`#15.年毎室温データ算出` → `15_CalcYearlyRoomTemperature.sh`) |
...
```sql
SELECT ConSensorDailyValues.c001, ConSensorDailyValues.c002, ConSensorDailyValues.c003
...
① Nếu total = 0 VÀ totalnumber = 0 (không có ngày nào có dữ liệu trong tháng) → BỎ, không ghi
...
kể cả những dòng bị BỎ ở bước 2.3/2.5.2 vì total=0 và totalnumber=0:
