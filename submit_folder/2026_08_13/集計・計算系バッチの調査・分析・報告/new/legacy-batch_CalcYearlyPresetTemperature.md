...
|---|---|---|
| `:calcDate` | `<yearmonth>-01` (dạng 'yyyy-MM-dd'; trong code parse '-01 00:00:00' rồi format chỉ còn phần ngày) | Khóa dòng-tháng cần đọc trong `s_103` (khớp cách `CalcMonthlyAverageSetTemperatureCommand` ghi: `datetime` = ngày-01 của tháng chứa ngày tính) |
| `:calcYear` | 4 ký tự đầu của `yearmonth` | Năm — dùng làm khóa dòng-năm khi ghi vào `s_104` |
| `:summaryThreshould` | Hằng số `NOT_SUMMARY_DATE_COUNT` = 10 | Ngưỡng loại: thiếu từ 10 ngày trở lên thì không tính (tối đa cho phép thiếu 9 ngày) |
...
s_103  "ConSensorDailyValue"    1 dòng/hộ/tháng  × 31 cột ngày   (device_type=17, room_id=0)
   ...
   │   CHỈ tính khi số ngày có dữ liệu > (số ngày trong tháng − 10), tức số ngày thiếu < 10)
   ▼
