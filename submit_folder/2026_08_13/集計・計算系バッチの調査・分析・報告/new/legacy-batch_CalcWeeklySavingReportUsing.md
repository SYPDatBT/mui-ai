...
| **Khái quát xử lý** | 1. Xác định "ngày tính" (`calcDate` — tham số đầu vào, hoặc ngày hiện tại nếu không truyền).<br>2. Query `s_103` lấy tổng lượng gas dùng (device_type=2) và tổng lượng điện dùng (device_type=5) của 7 ngày trước `calcDate`, theo từng hộ, kèm số ngày thực có dữ liệu.<br>3. Query `s_105` lấy các dòng report của ngày report gần nhất trước ngày tính (`MAX(c002)` chung toàn hệ, không phải MAX theo từng hộ), chuyển "lượng gas/điện tuần trước" của dòng đó thành "lượng gas/điện tuần trước nữa" của dòng sắp ghi.<br>4. Với mỗi hộ: nếu đủ 7 ngày dữ liệu gas và không thiếu dữ liệu giờ (check thêm ở `s_102`（日毎センサ情報）, cột 23h ngày trước `calcDate`) → tính lượng gas tuần trước (làm tròn) + giá trị hiển thị; tương tự cho điện (không có bước check giờ); thiếu điều kiện → null.<br>5. Ghi/cập nhật 1 dòng/hộ vào `s_105` (giữ nguyên các cột thuộc batch song hành), toàn bộ trong 1 transaction cho cả batch. |
...
| Command thực thi | `php cake.php CalcWeeklySavingReportUsing [算出日]` — theo docblock; thực tế code nhận qua option `--date=yyyy-MM-dd` (`buildOptionParser` :38), docblock ghi positional là mâu thuẫn của chính code cũ; sh vận hành không truyền tham số → luôn dùng ngày hiện tại | `CalcWeeklySavingReportUsingCommand.php:21-23, 38, 58`, `21_CalcWeeklySavingReportUsing.sh` |
...
```
① Lấy bản ghi s_102（日毎センサ情報; entity ConSensorHourlyValue）, device_type=2, room_id=0,
   cột 23時台 (c034) của ngày liền trước calcDate (dùng để phát hiện dữ liệu bị thiếu —
...
- Mỗi lần chạy: với **mỗi hộ xuất hiện trong bất kỳ kết quả nào của 3 query** (tính gas, tính điện, hoặc dòng report tuần trước ở SQL 3 — hộ chỉ có ở SQL 3 vẫn được ghi dòng mới chỉ chứa giá trị "tuần trước nữa" kế thừa), **INSERT/UPDATE (upsert) 1 dòng** với các cột: `used_last_week_gas → c011`, `used_last_week_ele → c012`, `used_week_before_last_gas → c013`, `used_week_before_last_ele → c014`, `report_used_last_week_gas → c021`, `report_used_last_week_ele → c022`, `report_used_week_before_last_gas → c023`, `report_used_week_before_last_ele → c024`; đặt cờ `GasEleCalculatedFlag (c041) = true`.
