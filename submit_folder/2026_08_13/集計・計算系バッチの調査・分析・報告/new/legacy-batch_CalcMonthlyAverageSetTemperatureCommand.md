...
`CalcMonthlyAverageSetTemperatureCommand` là batch chạy **1 lần/ngày, chỉ trong mùa sưởi ấm (khoảng 1/12 ~ 31/3)** trong hệ thống cũ (EMINEL コンシェルジュサーバー). Mỗi lần chạy, batch gộp **toàn bộ bản ghi trạng thái bộ điều khiển sưởi (北ガス1クラス 0F45) trong đúng 1 ngày** (nhiều lần đọc, không phân biệt phòng/thiết bị) thành **1 giá trị trung bình duy nhất** cho mỗi hộ — đại diện cho nhiệt độ cài đặt (設定温度) trung bình trong ngày đó. Batch chỉ đọc/ghi DB (không gửi mail, không xuất file), kết quả ghi vào bảng `s_103`. Tên batch ghi "月毎/Monthly" vì bảng đích lưu 1 dòng = 1 hộ × 1 tháng × 31 cột ngày — đó là đơn vị **lưu trữ**; đơn vị **tính** của batch này là 1 ngày (mỗi lần chạy chỉ tính và ghi đúng 1 cột ngày). Chi tiết lịch chạy, câu query, công thức tính và các batch dùng lại kết quả này trình bày ở Phần 2.

## Phần 1 — Tổng quát

| Mục | Nội dung |
|---|---|
| **Vai trò** | Gộp (nhiều→một) toàn bộ bản ghi trạng thái bộ điều khiển sưởi (北ガス1クラス 0F45) phát sinh trong 1 ngày thành 1 giá trị nhiệt độ cài đặt trung bình/hộ/ngày, để làm dữ liệu nguồn cho batch tổng hợp tháng và tính năng thưởng điểm "エコ暖房" (eco sưởi ấm) dựa trên hành vi cài nhiệt độ. |
...
| `t_202` | `c006` | Mã lớp thiết bị (EOJ) | Lọc `0F4500`〜`0F45FF`（lớp 北ガス1クラス（暖房制御）— lớp độc quyền Kitagas của bộ điều khiển sưởi, kiêm đo nhiệt/ẩm; cùng dải lớp mà `CalcTenMinutesSensorCommand` đọc giá trị cảm biến） |
...
- Mỗi lần chạy, với mỗi hộ có dữ liệu: set `device_type = ROOM_TEMP_SETTING (17)`, `room_id = 0`, `datetime` = ngày-01 của tháng chứa "ngày tính" (dùng làm khóa dòng-tháng), rồi set giá trị trung bình vào đúng cột ngày tương ứng (cột `c%03d` với số thứ tự = ngày trong tháng của ngày tính + 10, ví dụ ngày 05 → c015) và cập nhật `c051` (thời điểm sửa).
...
s_103  "ConSensorDailyValue"   1 dòng/hộ/tháng   × 31 cột ngày   (device_type=17, room_id=0)
   ...
   │ (chạy 16:10 ngày 1 các tháng 1–4 — 1 lần/tháng      │ (đọc thẳng dòng-tháng s_103
   │  ngay sau mỗi tháng mùa sưởi, gộp CÁC CỘT NGÀY       │  của 1 hộ để hiển thị)
   │  trong 1 tháng của s_103 thành 1 giá trị trung       │
   │  bình tháng, CHỈ tính khi số ngày THIẾU dữ liệu      │
   │  < NOT_SUMMARY_DATE_COUNT=10, tức số ngày có dữ      │
   │  liệu > số ngày trong tháng − 10; ngược lại ghi      │
   │  NULL vào cột tháng)                                 │
   ▼
