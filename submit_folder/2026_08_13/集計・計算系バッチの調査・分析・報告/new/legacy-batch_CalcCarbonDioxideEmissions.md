...
Bán điện (type 10) / Mua điện (type 11) — 2 nhánh song song cùng đổ vào s_102:
  (a) t_202 (raw ECHONET, chiều bán — chỉ hộ có pin mặt trời)
         │  CalcDailyAccumulatedValueCommand      (mỗi giờ; ghi type 10 khi hộ có pin mặt trời)
         ▼
      s_102
  (b) CSV Xzilla IF1156, 30 phút (hệ ngoài Xzilla cấp)
         │  RcvHalfHourElectricPowerCommand       (mỗi 10 phút — cron `*/10 * * * *`; ghi type 11 luôn, ghi type 10 khi hộ KHÔNG có pin mặt trời)
         ▼
      s_102
  s_102  (giờ, type 10/11)  ← hội tụ từ 2 nhánh (a)+(b)
     │  CalcMonthlyAccumulatedValueCommand        (mỗi ngày)
     ▼
