...
| **出力** | **DBへの書き込みのみ** ― 実行ごとに世帯あたり2件（センサー位置ごとに1件）を`s_102`（エンティティ`ConSensorHourlyValue`、共通ライブラリ`EminelSvLib`経由）へinsert-or-updateで書き込み、欠落時間帯があればbackfillの書き込みも追加する。メール送信・CSV出力は行わない。 |
...
② 末尾4文字が'7FFF'または'8000'（エラー値／センサー未接続）の場合 → このレコードを除外する
...
- 実行ごとに、**その時間帯に生データがある世帯ごと**に**レコード2件をinsert-or-updateで書き込む**（当日最初の実行時のみINSERT、以降は該当時間カラムのUPDATE） ― `room_id = 0`（E0）用に1件、`room_id = 1`（E1）用に1件。`device_type = 6`を設定し、`date`は`calculationTime`の日付、該当する時間カラム（`c0XX`、XX＝時＋11）には平均値（またはnull）を設定、`need_ele_complete_flag = 2`、`need_agg_complete_flag = 2`とし、`t_101`から取得した5つの属性グループも併せて設定する。
...
2. 各世帯について：
   ...
   b. 1時間ずつ遡る（subHour = 1 → 168、つまり最大7日分）：
      - その時間カラムがNULLのまま、かつ0時でない場合 → 何も書き込まず、さらに1時間遡る
      - 値が既にある（NULLでない）カラム、または0時に達した場合 → その時刻からcalculationTimeまでの
        t_202の生データを欠落区間分まとめて一度に取得し                              (getHourlyData)
        （注：backfillはc003='EA'のみで、本体SQLの'EA'/'EB'と異なる）
      ...
      - 生データの日付が算出対象日より古い場合（データ遅延）はneed_agg_complete_flag = 1、
        同日の場合は= 2とする
      - その後、その時刻が0時の場合 → 前日分のs_102の行を再取得し、さらに遡り続ける。
        そうでない場合 → 終了し、次の世帯へ
```
