...
① Nếu total = 0 HOẶC totalnumber = 0 → BỎ, không ghi (code chỉ ghi khi cả total lẫn totalnumber cùng khác 0 — lưu ý: ngày có dữ liệu nhưng tổng đúng bằng 0 cũng bị bỏ qua)
...
```sql
-- Cấu trúc SQL giống mục 2.2, nhưng SELECT thêm cột ConSensorHourlyValues.c004 (ngày của dòng dữ liệu) và khác điều kiện lọc ngày:
...
