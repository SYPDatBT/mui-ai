...
| Đích | `s_102`（`ConSensorHourlyValues`）, ghi đè theo cột giờ `c011`~`c034`（00時台=c011 … 23時台=c034）; `measurement00`~`measurement23` chỉ là alias SELECT của SQL gộp, không phải tên cột vật lý của `s_102` |
| Phạm vi quét lại | **193 giờ** (~8 ngày) lùi về trước mỗi lần chạy, chỉ gọi lại SQL khi **ngày** thay đổi (9 lần/lần chạy — 193 giờ phủ 9 ngày lịch) | `CalcDailyEnergyConsumptionCommand.php:452-480` |
...
| 6 | 売電 (bán điện) | 10 | `CalcDailyAccumulatedValueCommand` **hoặc** `RcvHalfHourElectricPowerCommand` | Tùy `c034`(pin mặt trời) — có pin mặt trời thì lấy từ ECHONET (đo chiều ngược); không có pin mặt trời thì lấy từ Xzilla **chỉ khi** `c024 = 1`（コレモ）và `c064`（受電地点特定番号）có giá trị, ngoài ra 売電 được ghi cố định `0` |
