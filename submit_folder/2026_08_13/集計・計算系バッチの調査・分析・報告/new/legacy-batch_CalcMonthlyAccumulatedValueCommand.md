...
`room_id` được lọc cứng bằng `DETECT_LIVING(0)` bất kể `type` — vì trong `s_102` chỉ `DETECT_CNT` (14) và 室内温度 (6) — đều không nằm trong danh sách tháng — mới dùng `room_id` khác 0, nên với 11 loại của batch này điều kiện luôn đúng.
...
2. Với mỗi hộ: getRecountDateTime dò ngược từng ngày, bắt đầu từ
   ...
   - nếu dòng-tháng trước không tồn tại trong s_103 → dò dừng sớm,
     điểm bắt đầu tái tính = ngày 1 của tháng đang dò; ngoài ra vòng dò
     so sánh số tháng thuần nên không hoạt động khi khoảng dò vắt qua
     ranh giới năm (tháng 1)
...
5. updateRetroactiveFlag: cập nhật cờ c009 = 2 (đã tổng hợp) trên các
   dòng-giờ s_102 trong khoảng [điểm bắt đầu, ngày tính] — rộng hơn
   khoảng ghi lại 1 ngày, bao luôn dòng-giờ của ngày tính vừa được bước
   2.3 tổng hợp — chỉ những dòng đang có c009 = 1 (đang chờ tổng hợp).
   Bước 4-5 nằm trong 1 transaction/hộ.
```
