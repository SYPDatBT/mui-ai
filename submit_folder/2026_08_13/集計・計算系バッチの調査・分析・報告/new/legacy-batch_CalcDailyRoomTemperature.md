...
| **Output** | **Chỉ ghi DB** — mỗi lần chạy ghi 2 dòng/hộ (1/vị trí cảm biến, kiểu insert-or-update) vào `s_102`（entity `ConSensorHourlyValue`, qua thư viện chung `EminelSvLib`), cộng thêm các bản ghi backfill nếu có giờ trống. Không gửi mail, không xuất CSV. |
...
② Nếu 4 ký tự cuối là '7FFF' hoặc '8000' (giá trị lỗi/chưa gắn cảm biến) → BỎ bản ghi này
...
- Mỗi lần chạy: với **mỗi hộ có dữ liệu thô trong giờ**, ghi **2 dòng theo kiểu insert-or-update** — 1 cho vị trí `room_id = 0` (E0), 1 cho `room_id = 1` (E1); lần chạy đầu tiên trong ngày INSERT dòng mới, các giờ sau UPDATE dòng hiện có của ngày đó — set `device_type = 6`, `date` = ngày của `calculationTime`, set thêm cột giờ tương ứng (`c0XX`, XX = giờ + 11) = giá trị trung bình (hoặc null), `need_ele_complete_flag = 2`, `need_agg_complete_flag = 2`, cùng 5 thuộc tính nhóm lấy từ `t_101`.
...
2. Với mỗi hộ:
   ...
   b. Lùi từng giờ (subHour = 1 → 168, tức tối đa 7 ngày):
      - Nếu cột giờ đang NULL và không phải giờ 0 → bỏ qua, lùi tiếp 1 giờ (chưa ghi gì)
      - Khi gặp cột đã có giá trị (không NULL) hoặc giờ 0 → lấy dữ liệu thô t_202 trong khung
        (giờ đó, calculationTime) một lần cho cả khoảng trống                        (getHourlyData)
        (lưu ý: backfill chỉ lấy c003='EA', khác SQL chính lấy 'EA'/'EB')
        rồi ghi từng bản ghi thô vào đúng cột giờ của ngày tương ứng                 (updateRecalculationData)
      ...
      - need_agg_complete_flag = 1 nếu ngày bản ghi thô cũ hơn ngày đang tính (dữ liệu trễ),
        = 2 nếu cùng ngày
      - Sau đó nếu giờ đó = 0 → lấy dòng s_102 ngày trước, lùi tiếp;
