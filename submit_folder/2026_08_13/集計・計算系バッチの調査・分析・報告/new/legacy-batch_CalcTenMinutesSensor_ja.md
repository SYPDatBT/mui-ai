...
**SQLで使用するカラムの意味**（c248/c249はコード内コメント（`CalcTenMinutesSensorCommand.php:122-136`）、その他のカラムは共通ライブラリのエンティティ定数（`eminel_sv_lib` `ConDeviceStatus.php:281-286`・`ConCustomer.php:49,75`）より）：
...
③ 16進数 → 10進数に変換
④ コード上の範囲チェックは『0未満 かつ 999,999,999超』（&&条件、CalcTenMinutesSensorCommand.php:189-190）で、
   論理的に成立し得ないデッドコード（旧システムの潜在バグ）。
   実効的にnullになるのは 元の文字列 = 'FFFFFFFF'（センサー関連付け無し）／'FFFFFFFE'（センサー無応答）のみ。
   範囲外値は⑤の差分閾値（>300）で間接的に弾かれる場合を除き通過する。
   新システムで再実装する際はOR条件に直すか踏襲するかの判断が必要。
⑤ それ以外:
       差分 = （枠Bの読み値） − （枠Aの読み値）
...
- 実行毎に、**有効な各戸**について**2レコード**を書き込む — `DETECT_LIVING` 用に1件、`DETECT_OTHER` 用に1件 — `device_type = DETECT_CNT(14)`、`room_id`、`date_time = T`、`measured_value = 差分（またはnull）` を設定。※nullの場合、コード上は `(string)$calcValues` により空文字''として保存される（`CalcTenMinutesSensorCommand.php:212`）。
