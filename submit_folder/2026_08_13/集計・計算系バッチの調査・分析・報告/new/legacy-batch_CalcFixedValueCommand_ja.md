...
| **Output** | **DBへの書き込みのみ**――データが返却された世帯・日ごとに、`s_103`（エンティティ`ConSensorDailyValue`、共通ライブラリ`EminelSvLib`経由）へ1レコードを（UPSERTとして：主キーで既存があればUPDATE、なければINSERT）書き込む：`device_type = BUY_ELECTRIC(11)`、対応する日付カラム1つ（`c011`〜`c041`）＋グループ属性カラム5つをセットする。メール送信・CSV出力は行わない。 |
...
- `save()`を呼び出す――CakePHP（4.4）は、主キー4カラムが揃った新規エンティティに対してデフォルトの`checkExisting`によりPKでの存在確認を自ら行い、レコード（世帯、11、0、年月）が既に存在すれば該当日付カラムをUPDATE、存在しなければINSERTする――すなわち**UPSERT**であり、旧システムのdocblockが「INSERT(UPSERT)」と呼ぶパターンと同一である（`CalcMonthlyAccumulatedValueCommand.php:638`）。Commandのコード自体は既存レコードを読み込まず、存在確認はORMが行う。
