...
売電（type 10）／買電（type 11）:
  t_202（生ECHONET、売電方向 ― 太陽光パネルがある世帯のみ）
     │  CalcDailyAccumulatedValueCommand          （毎時；太陽光パネルがある世帯の場合にtype 10を書き込む）
     ▼
  CSV Xzilla IF1156、30分
     │  RcvHalfHourElectricPowerCommand           （毎時；type 11は常に書き込み、太陽光パネルが無い世帯の場合はtype 10も書き込む）
     ▼
  s_102  （時間、type 10/11）
     │  CalcMonthlyAccumulatedValueCommand        （毎日）
     ▼
