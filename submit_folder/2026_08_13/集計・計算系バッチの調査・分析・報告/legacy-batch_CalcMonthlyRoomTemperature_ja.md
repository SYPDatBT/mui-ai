...
`CalcMonthlyRoomTemperatureCommand`は、旧システム（EMINEL コンシェルジュサーバー）内で1日1回実行されるバッチであり、1日分の**24個の時間別平均室温**（`s_102`テーブルに既に格納済み。別バッチが算出）を、世帯×センサー位置（E0/E1）ごとに**1つの日次平均値**へ集約し、月次テーブル`s_103`（1行につき31カラム＝月内の日ごとに1カラム）へ書き込む。算出対象日に加えて、バッチは当月・前月分の時間データ全体を再スキャンし、前回算出後にソースデータが変更・追加された日を**再計算**したうえで、ソースである`s_102`を「集計済み」として再度マークする。バッチはDBの読み書きのみを行う（メール送信・ファイル出力は行わない）。実行スケジュール、SQL文、計算式、業務定数の詳細は第2部に記載する。
...
① total = 0 かつ totalnumber = 0（その日にデータが存在する時間が1つもない）の場合 → スキップし、書き込まない
...
① total = 0 かつ totalnumber = 0 の場合 → スキップし、書き込まない
② それ以外の場合 → 日次平均 = total / totalnumber
