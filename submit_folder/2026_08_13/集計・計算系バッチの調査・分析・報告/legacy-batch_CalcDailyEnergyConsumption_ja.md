...
| 書き込み先 | `s_102`（`ConSensorHourlyValues`）、時間カラム`c000`~`c023`（`measurement00`~`measurement23`）に上書き |
| 再走査範囲 | 実行のたびに**193時間**（約8日間）分を遡る。SQLの再実行は**日付**が変わるタイミングのみ（1回の実行あたり約8回） | `CalcDailyEnergyConsumptionCommand.php:452-480` |
...
| 6 | 売電 | 10 | `CalcDailyAccumulatedValueCommand` **または** `RcvHalfHourElectricPowerCommand` | `c034`（太陽光パネル）による — 太陽光パネルがある場合はECHONETから取得（逆方向計測）、ない場合はXzillaから取得 |
