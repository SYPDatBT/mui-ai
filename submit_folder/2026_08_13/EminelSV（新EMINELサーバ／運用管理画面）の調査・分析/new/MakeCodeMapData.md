...
| Mục | Nội dung | Nguồn |
|---|---|---|
| Lịch chạy (cron) | **Không có.** Grep tên class trong cả 2 file cấu hình cron (`webap_cron設定_20240905.txt`, `mng-webap_cron設定_20241029.txt`), trong `cron設定概要.txt` và trong toàn bộ shell script giải nén từ `cron実行用シェルスクリプト/*.tgz` — đều không có kết quả; riêng `cron設定一覧.xlsx` là binary chưa đọc được (giới hạn còn lại). | — |
...
**Ví dụ thật — trích đầu file output `config/code_map_data.txt` có sẵn trong repo** *(CSV input không
còn trong repo, nhưng file output chứa dữ liệu thật đã serialize, xem phần "đã tra" dưới đây)*:

```
Đầu file code_map_data.txt (chuỗi PHP serialize, 1.686 category):
a:1686:{s:4:"0006";a:9:{i:1;s:39:"センターポーリング毎日検針";i:2;s:12:"随時検針";...}
        s:7:"C011320";a:8:{i:1;s:18:"一般ガス事業";i:2;s:27:"ＬＰＧ一般ガス事業";...}...}

→ tức $mapData lúc được dựng có dạng:
[
...
**Đã tra để xác định CSV gốc là loại code nào — CSV input không còn trong repo, nhưng file output thật
trả lời được một phần:**
- `ls config/` trong `eminelsv-develop` → chính file output `code_map_data.txt` (2.781.482 byte, dữ liệu
  thật đã serialize) nằm sẵn tại đường dẫn mặc định — chứng minh batch từng được chạy thực tế; nội dung
  là các bảng mã hệ 基幹 (Xzilla-系): ガス事業種別 (`C011320`, `0010001`), 都道府県 (`K101900`),
  開栓／閉栓／撤去 (`K001280`〜`K001282`), 検針 (`0006`), 電池電圧 (`0033`), dải 延べ床面積 (`004`:
  「～70m2」…「151m2～」) — trùng đúng dải thuộc tính グルーピング trong requirement v1.2 dòng 619.
...
- Có hỏi xác nhận ghi đè qua console **khi file đích đã tồn tại**; muốn chạy không tương tác thì trỏ
  `OUTPUT_FILE` sang đường dẫn chưa tồn tại, hoặc pipe sẵn câu trả lời vào stdin. (`MakeCodeMapDataCommand.php:44-52`)
- File output thật (2.781.482 byte, dữ liệu serialize thật) có sẵn ngay trong repo tại đường dẫn mặc
  định — batch từng được chạy thực tế. Grep toàn bộ `sources/eminelsv-develop` các chuỗi
  `code_map_data`/`codeMapData`/`CODE_MAP`, và `unserialize` trong toàn bộ `*.php` trên `sources/` —
  không tìm thấy chỗ nào đọc lại file này. *(suy đoán: có thể tiêu thụ ngoài repo, hoặc là công cụ hỗ
  trợ vận hành/migrate 1 lần, không có consumer runtime)*.
...
- File output thật cho thấy CSV input là các bảng mã hệ 基幹 (ガス事業種別・都道府県・開閉栓・検針・
  延べ床面積区分…); ở E-GW các mã kiểu này đã được quản bằng cơ chế master của e-smart (seed
  `AttributeMasterTable` + API) — câu hỏi còn lại là có cần bổ sung category nào cho E-GW không.
...
| Hệ thống cũ | Cron (xác nhận không có) | `docs/02_詳細設計/10_バッチ処理/webap_cron設定_20240905.txt`, `mng-webap_cron設定_20241029.txt`, `cron設定概要.txt`, `cron実行用シェルスクリプト/*.tgz` (đã giải nén kiểm shell script; `cron設定一覧.xlsx` binary chưa đọc được) |
| Hệ thống cũ | File output thật (dữ liệu serialize) | `sources/eminelsv-develop/config/code_map_data.txt`; đối chiếu dải 延べ床面積: `eminel_gw_project/docs/eminel/3_requirements/00_integrated_requirements_v1.2.md:619` |
