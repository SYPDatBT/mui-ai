...
| Mục | Nội dung |
|---|---|
| **Vai trò hiện tại** | Phân phối nội dung **do admin tự soạn** (Tip/News/Survey) tới người dùng theo thuộc tính tĩnh hoặc danh sách CSV — trong khuôn khổ phân phối nội dung này không có nghiệp vụ nhắm theo điều kiện tính toán động; riêng cảnh báo 熱中症/見守り có pipeline push theo giá trị cảm biến (`batch-receive-data-infrared-remote` → `batch-control-device-and-push-notice-sensor`) nhưng không phải lời khuyên tiết kiệm năng lượng. |
...
| Pipeline cảnh báo 熱中症/見守り theo cảm biến | `syp-eminelstandard-backend-main/src/functions/batch-receive-data-infrared-remote/app.ts:200-225`, `src/functions/batch-control-device-and-push-notice-sensor/app.ts:64-97` |
