...
| `batch-end-dr` (+ `-preprocessing`) | `batch-end-dr/app.ts` | Tại thời điểm DR kết thúc: cấp điểm/badge tham gia DR nếu cấu hình có (`givePointBadgeForUser`, khớp `POINT_BADGE_FOR.DR`), rồi **khôi phục thiết bị về trạng thái đã lưu** (tắt trước, mở sau — theo đúng thứ tự `offDevices` rồi `onDevices`) |
...
| `create-dr.ts` / `update-dr.ts` | `src/functions/api-dr/` | Tạo/sửa lịch DR — **chỉ đăng ký EventBridge Scheduler one-shot cho giờ 配信 (`SEND_DR` theo `send_time`)**; đến giờ đó chuỗi `batch-send-dr` phát DR tới user (`TABLE_DR_USER_ACTION`), rồi `batch-send-dr-complete` đăng ký tiếp one-shot `START_DR`/`END_DR` đúng giờ thực thi (`implement_start_time`/`implement_end_time`) + ghi `TABLE_DR_STATS` — vẫn one-shot, không polling mỗi phút như hệ cũ |
...
| **Output** | Gọi API cloud hãng (Rinnai/Noritz/Daikin) hoặc dịch vụ hồng ngoại MUI để điều khiển thật ＋ cập nhật `latest_control_by`/`latest_control_id` vào `TABLE_DEVICE`/`TABLE_MUI_DEVICE` ＋ `TABLE_DR_USER_ACTION.pre_control_status` (trạng thái trước điều khiển, dùng để khôi phục) ＋ điểm/badge qua `givePointBadgeForUser` (ghi `TABLE_POINT_BADGE_STATS`・`TABLE_USER_BADGE_SUMMARY`・`TABLE_SYSTEM_STATS`). |
| **Khái quát xử lý** | 1. Tạo/sửa DR qua `api-dr` → đăng ký one-shot `SEND_DR` theo `send_time`; đến giờ 配信, chuỗi `batch-send-dr` phát DR tới user rồi `batch-send-dr-complete` đăng ký one-shot `START_DR`/`END_DR` đúng giờ thực thi (không polling).<br>2. Lúc bắt đầu (`batch-start-dr`): với mỗi user, lấy thiết bị theo loại (Eco Jozu/điều hòa/fan convector), **lưu trạng thái hiện tại**, rồi điều khiển theo `control_setting` (ON/OFF/đổi nhiệt độ).<br>3. Lúc kết thúc (`batch-end-dr`): cấp điểm/badge nếu có cấu hình (`point_quantity>0` hoặc `has_badge`), rồi đọc lại trạng thái đã lưu, **khôi phục** thiết bị (tắt trước, mở sau). |
...
| Nội dung | Căn cứ |
|---|---|
| Điều khiển lúc bắt đầu DR | `syp-eminelstandard-backend/src/functions/batch-start-dr/app.ts` |
| Khôi phục + cấp điểm lúc kết thúc DR | `syp-eminelstandard-backend/src/functions/batch-end-dr/app.ts` |
| Hàm điều khiển thiết bị dùng chung (DR + Automation) | `syp-eminelstandard-backend/src/layers/common/nodejs/business-logic/control-device.ts` |
| Tạo lịch DR + đăng ký EventBridge Scheduler | `syp-eminelstandard-backend/src/functions/api-dr/create-dr.ts`, `update-dr.ts`, `src/functions/batch-send-dr-complete/app.ts` |
