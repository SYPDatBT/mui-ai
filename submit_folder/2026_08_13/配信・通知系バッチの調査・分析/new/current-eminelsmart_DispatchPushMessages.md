...
| **Input** | Danh sách người nhận + nội dung đã build sẵn trên S3 (do preprocessing của từng loại nội dung tạo) ＋ `TABLE_MOBILE_TOKEN_MANAGEMENT` (token theo user, nhiều thiết bị) ＋ `TABLE_USER_SETTING` (cờ bật/tắt thông báo theo `target_screen`: Tip/Survey/News/DR/lỗi thiết bị/giám sát phòng/cảnh báo sốc nhiệt — Tip/Survey/News/DR dùng chung 1 cờ `flag_push_notice_tab_notifications`, tổng cộng 4 cờ, khớp D03 「ESTAは4フラグ」; đơn vị tách chi tiết là việc của 仕様詳細). |
...
| **Khái quát xử lý** | 1. Mỗi loại nội dung (Tip/News/Survey/DR) tự có state machine riêng: preprocessing build danh sách người nhận → chia segment → gọi `batch-push-notice` song song.<br>2. Với mỗi user trong segment: kiểm tra cờ bật/tắt theo loại nội dung, bỏ qua nếu tắt.<br>3. Lấy tất cả token của user, gửi Firebase trực tiếp cho từng token, giới hạn 100 request đồng thời cho mỗi Lambda instance (mỗi segment); với Map `MaxConcurrency=5`, một state machine có thể tới 5×100 request đồng thời.<br>4. Lỗi từng token: nếu do token hết hạn/không hợp lệ → xoá token; lỗi khác → chỉ log, **không retry lại**. |
...
| Cơ chế trigger | 1 dispatcher chung, polling mỗi phút theo bảng lịch (`PushMessageDestinations`) — trừ ngoại lệ gửi thẳng (`TerminateOutdatedDeviceControlJobsCommand`, hemssv `dev_ctrl`) | Mỗi loại nội dung tự trigger ngay khi preprocessing xong (không polling theo lịch cố định) |
| Retry khi gửi thất bại | Có — tối đa 5 lần, cách nhau 3 phút, lưu trạng thái từng đích | **Không có** — lỗi chỉ log, không gửi lại |
| Bật/tắt theo loại thông báo | Không có ở dispatcher; ở khâu phát, hệ cũ có bật/tắt cho loại みまもり (`ConMotionSensorNotificationSettings`) — các loại khác không thấy | **Có** — `TABLE_USER_SETTING` theo từng `target_screen` |
