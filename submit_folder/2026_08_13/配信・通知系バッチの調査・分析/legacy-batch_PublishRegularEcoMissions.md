...
| 19 | Ngày 2 hàng tháng | `StartContractAnniversaryPublisher` | "Cảm ơn đã dùng EMINEL %%YEARS%% năm" | 100 |
...
| Mục | Nội dung |
|---|---|
| Dữ liệu xét | `ConSensorMonthlyValues` loại `device_type=18` (`TOTAL_CO2_EMISSIONS`), `room_id=0`, 2 năm liên tiếp (năm tài chính hiện tại và năm trước) ([:83-91](e:/Projects/mui/legacy_eminel_docs-main/sources/conciergesv-develop/src/Command/PublishRegularEcoMission/Co2ReducedPublisher.php#L83-L91)) |
...
| `OverGasElectricUsageOverAvgWinterPublisher` — ID 7-10 | `3` (nước nóng) | **Chỉ áp dụng hộ đã tách sưởi/nước nóng**: `heater_ctrl_mode=AT` (tự động), nguồn nhiệt nước nóng ∈{13A,LPG,dầu}, nguồn nhiệt nước nóng = nguồn nhiệt sưởi, ≠ nguồn nhiệt loại thứ ba ([:89-97](e:/Projects/mui/legacy_eminel_docs-main/sources/conciergesv-develop/src/Command/PublishRegularEcoMission/OverGasElectricUsageOverAvgWinterPublisher.php#L89-L97) — cùng 6 điều kiện `separate_calc_target` đã gặp ở `CalcTenMinutesEnergyCommand`) | Mùa đông (1,2,3,4,5,11,12) |
...
|---|---|---|---|
| 13 | `SetHighTempPublisher` | Nhiệt độ cài đặt **mode ở nhà** (HS 開始種別=`33`) từ 9:00 hôm đó tới 9:00 hôm sau, trung bình **≥ 23°C** ([:37, :85](e:/Projects/mui/legacy_eminel_docs-main/sources/conciergesv-develop/src/Command/PublishRegularEcoMission/SetHighTempPublisher.php#L37-L85)) | `ConDeviceStatuses` EPC `A1`, chỉ hộ `heater_ctrl_mode=AT` |
| 14 | `EcoModeNotSetPublisher` | Nhiệt độ cài đặt **ECO mode** tại đúng 12:00 ngày 20 = `0` (chưa cài) ([:36-57](e:/Projects/mui/legacy_eminel_docs-main/sources/conciergesv-develop/src/Command/PublishRegularEcoMission/EcoModeNotSetPublisher.php#L36-L57)) | `ConDeviceStatuses` EPC `A7` |
...
| 16 | `SetHighTempInAbsencePublisher` | Nhiệt độ phòng khách (12h-14h) trung bình **≥ 20°C** trong 2 tuần liên tiếp **và** không có phát hiện chuyển động lần nào trong cả 2 tuần đó ([:35-98](e:/Projects/mui/legacy_eminel_docs-main/sources/conciergesv-develop/src/Command/PublishRegularEcoMission/SetHighTempInAbsencePublisher.php#L35-L98)) | `ConSensorHourlyValues` (device_type `6`=nhiệt độ phòng, `14`=phát hiện chuyển động) |

Cả 4 đều yêu cầu `heater_ctrl_mode=AT` (trừ ID 16), và đều dùng field EPC thô từ `ConDeviceStatuses` (bảng ECHONET raw, cùng bảng dùng ở `CalcTenMinutesEnergyCommand`) — không qua bảng tổng hợp trung gian.
...
| `ConEcoMissionDestinations` | 1 dòng/EMS-SP thoả điều kiện — đây là nguồn app đọc để hiển thị "nhiệm vụ của tôi". **ID 1 không tạo dòng nào ở bảng này** (xem §2.3) |
...
**① ID 1 không đi qua cơ chế "nhiệm vụ theo từng khách hàng".** Toàn bộ 18 ID khác đều tạo `ConEcoMissionDestinations` cho từng EMS-SP thoả điều kiện — đây là nơi app đọc để biết "tôi có nhiệm vụ nào". Riêng ID 1 chỉ gửi Push qua topic chung, **không** ghi `ConEcoMissionDestinations` — nếu app hiển thị danh sách nhiệm vụ dựa trên bảng này (nhiều khả năng đúng vậy, vì đó là mục đích bảng này tồn tại), mission ID 1 sẽ **không** xuất hiện trong danh sách đó dù đã gửi Push.
