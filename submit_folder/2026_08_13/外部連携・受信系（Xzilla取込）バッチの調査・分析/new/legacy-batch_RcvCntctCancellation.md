...
| Điều kiện lọc file | Chỉ nhận file có chứa chuỗi `.csv` trong tên (`str_contains` — không phải kiểm tra đuôi chặt) ([:83](e:/Projects/mui/legacy_eminel_docs-main/sources/conciergesv-develop/src/Command/RcvCntctCancellationCommand.php#L83)) |
...
|---|---|---|
| `0` | Đang xử lý (hằng số `XZILLA_RELATION_LOGS_STAUS_PROCESSING = 0`, định nghĩa trong component dùng chung — [XzillaRelationComponent.php:28-29](e:/Projects/mui/legacy_eminel_docs-main/sources/conciergesv-develop/src/Controller/Component/XzillaRelationComponent.php#L28-L29)) | Dừng batch (`commit` + `abort`) |
...
- Ghi log `file_name=<file>` với trạng thái "đang xử lý" trước khi upsert dữ liệu, lấy `xzillaRalationLogsInsertId` để dùng lại ở bước hoàn tất ([:158-164](e:/Projects/mui/legacy_eminel_docs-main/sources/conciergesv-develop/src/Command/RcvCntctCancellationCommand.php#L158-L164)). Lưu ý: nhánh update của `saveXzillaRelationLogs` chỉ khớp record `upload_type = 1` do hardcode ([XzillaRelationComponent.php:125](e:/Projects/mui/legacy_eminel_docs-main/sources/conciergesv-develop/src/Controller/Component/XzillaRelationComponent.php#L125) — nghi bug hệ cũ); với file IF2249 thực tế luôn rơi vào nhánh insert mới, ghi `upload_type=3`.
- Lỗi ở bước tải file → `commit` + `abort`, không rollback (chưa có gì để huỷ) ([:150-153](e:/Projects/mui/legacy_eminel_docs-main/sources/conciergesv-develop/src/Command/RcvCntctCancellationCommand.php#L150-L153)); lỗi ghi log "đang xử lý" → `rollback` toàn bộ transaction ([:160-163](e:/Projects/mui/legacy_eminel_docs-main/sources/conciergesv-develop/src/Command/RcvCntctCancellationCommand.php#L160-L163)).
...
| `c066 = 0` | Còn được phép cập nhật — `c066` là cờ ức chế cập nhật (`C_UPDATE_DETER_FLAG`, [ConCustomer.php:81-82](e:/Projects/mui/legacy_eminel_docs-main/sources/eminel_sv_lib-develop/src/Model/Entity/ConCustomer.php#L81-L82); cùng chỗ xác nhận `c065` = `C_SELL_BUY_CALC_STOP_FLAG`) — ngữ nghĩa nghiệp vụ chi tiết vẫn là *suy đoán* |
...
| Trường hợp | Hành vi |
|---|---|
| Không có file / không có file hôm nay / file đã xử lý (§2.1, §2.2) ・ lỗi tải file (§2.3) ・ lỗi tra log IF2264 (§2.7) | `commit` rồi `abort` — không rollback (riêng §2.7: giữ nguyên phần đã upsert) |
| Lỗi ở bước ghi log "đang xử lý" (§2.3), upsert, update cờ, ghi log hoàn tất, gọi API (§2.4–§2.7) | `rollback` toàn bộ rồi `abort` |
...
| Hằng số (đường dẫn local, loại file, status log) | 4 hằng số của batch định nghĩa ngay trong class này ([:29-36](e:/Projects/mui/legacy_eminel_docs-main/sources/conciergesv-develop/src/Command/RcvCntctCancellationCommand.php#L29-L36)); riêng `HTTP_STATUS_200` dùng ở bước gọi API (§2.7) lấy từ file hằng số dùng chung [config/const.php:137](e:/Projects/mui/legacy_eminel_docs-main/sources/conciergesv-develop/config/const.php#L137) |
