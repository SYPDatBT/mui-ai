...
Lý do hợp lý: dữ liệu điện 30 phút được Xzilla gửi nhiều lần/ngày (theo báo cáo di trú: 10分毎に速報／確報riêng biệt), nên 1 lần chạy batch cần bù hết các file mới chưa xử lý, không chỉ 1 file.
...
| `1` | `lct_ctgr` | 供給_受電区分 — `1`=供給 (điện bán từ lưới cho khách/mua điện), `2`=受電 (lưới nhận điện từ khách/bán điện) |
...
**② `emn_confirm_electric_powers` tích lũy vô hạn.** Đây là bảng duy nhất trong 3 bảng liên quan tới CSV không bị `deleteAll` mỗi lần chạy — trong khi `emn_all`/`emn_fast` luôn xoá‑nạp lại toàn bộ.
