...
đều khác bản chất — chúng generate dữ liệu mới từ DB hoặc đi chiều ngược (nhận file từ đối tác khác vào
S3), không phải đọc lại file zip log app đã có sẵn để giải nén/validate/gửi ra Xzilla. Chức năng "gửi log
...
| **Khái quát xử lý** | 1. Xác định ngày filter (tham số `--datetime` hoặc hôm qua). 2. Kết nối SFTP bằng private key. 3. Liệt kê file `.zip` local trong thư mục `xzilla`, lọc theo ngày filter (parse từ tên file). 4. Với mỗi zip khớp: giải nén ra thư mục tạm hệ thống. 5. Với mỗi `.tsv` trong đó: validate format, rồi SFTP PUT nếu hợp lệ. 6. Dọn thư mục tạm + xóa zip local sau khi xử lý xong 1 zip. |
...
3. Với mỗi file `.tsv`, validate bằng `validateTsvFile()` (`:94, 125-152`):
   ...
   - Từ dòng thứ 2, mỗi dòng không trống phải khớp regex `^\d{4}\/\d{2}\/\d{2}` (bắt đầu bằng ngày dạng `YYYY/MM/DD`); nếu 1 dòng bất kỳ không khớp → toàn file bị coi là không hợp lệ, dừng kiểm tra ngay (`break`) (`:137-147`).
...
- Batch đọc file input do 1 **API khác** (`GetLogFileController`) tạo ra trước đó, không tự sinh dữ liệu — khi port cần đảm bảo có luồng tương đương ghi file/dữ liệu vào "hộp thư" trước khi batch gửi đi được kích hoạt, nếu không sẽ luôn rơi vào nhánh "không có file" (`PutLogFileCommand.php:52-55`, log `alert` "指定された日に該当するファイルがありません。" rồi `return` sớm — không coi là lỗi).
...
| `src/functions/batch-export-kyutoki-accumulated`, `-daily-usage`, `-device-property`, `-device-status-history`, `-monthly-usage`, `batch-export-sekigaisen-rimokon` | Có SFTP PUT thật (`src/layers/common/nodejs/services/upload-data-backup-to-sftp.ts`, secret `sftp_server_info_eminel`, đích `/EST`), nhưng dữ liệu được **generate mới từ DB** (query thiết bị rồi convert CSV), không đọc lại file zip log app có sẵn, không có bước giải nén/validate TSV theo header+ngày. Đích và secret khác hẳn `PUT_LOG_TARGET_DIR_PATH`/`XZILLA_RELATION_SERVER_HOST`. Khác bản chất — chỉ trùng ở mức kỹ thuật "SFTP PUT ra ngoài". |
| `src/functions/batch-forward-csv-from-sftp-server-to-s3`, `batch-get-list-file-name-from-sftp-server`, `batch-delete-data-temp-sftp` | Đi **chiều ngược lại**: đọc file `.dat` (IF2241/DM1040/IF2242/IF2016/IF2023/IF2024/IF2029/IF2223) TỪ SFTP server đối tác VÀO S3 để import, không phải gửi log app RA Xzilla. Khác bản chất về hướng dữ liệu và về hệ thống đối tác (không phải Xzilla theo tên biến/secret dùng). |
