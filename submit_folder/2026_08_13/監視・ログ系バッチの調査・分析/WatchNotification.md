...
| **Input** | Đọc từ 4 bảng DB: `t_901`（cấu hình + lịch sử thông báo trước）＋ `t_101`（danh sách hộ）＋ `t_201`（thiết bị đã đăng ký, lọc cảm biến người）＋ `s_101`（dữ liệu PIR 10 phút, do `CalcTenMinutesSensorCommand` tạo ra trước đó）. Không đọc CSV/file, không gọi API ngoài. |
| **Output** | Insert `PushMessage`(Push notification) + `ConMessage`("お知らせ" trong app) — 1 cặp bản ghi cho mỗi hộ có trạng thái ≠ "không áp dụng". Update `t_901` (thời điểm thông báo trước) cho mọi hộ đã xét. |
...
- Lấy tổng số bản ghi **thiếu** (`c006 IS NULL` → tính là 1) trong `CONSECUTIVE_MISSING_VALUES − 1 = 2`
  khung 10 phút gần nhất, riêng cho từng phòng (`c006Living`, `c006Other`).
...
│
├─ count > 0 VÀ thời điểm phán đoán (làm tròn 10p) == giờ kết thúc theo dõi (làm tròn 10p)
...
**Đọc kỹ**: nhánh "không có người"/"không có dữ liệu" **chỉ được xét đúng vào khung giờ kết thúc theo
dõi** — các khung giữa chừng chỉ có thể ra "có người" hoặc "không áp dụng", không báo "không có người"
giữa giờ.

### A.2.6 Thuật toán 見守り② — "見守り通知" theo dõi liên tục (`checkMonitoring2`, khi `c003_02` bật)

Ý tưởng: xét **nhiều khung 10 phút** trong toàn bộ thời gian theo dõi (không chỉ 1 khung), dùng số
khung tối thiểu (`SENSOR_NOTIFY_JUDGMENT_FRAME_NUMBER = 3`, tức 30 phút) để tránh báo động giả từ 1 lần
đo lẻ.

```
monitorFrameNum = (giờ kết thúc − giờ bắt đầu theo dõi) / 10 phút
frameNum        = MIN(SENSOR_NOTIFY_JUDGMENT_FRAME_NUMBER=3, monitorFrameNum)
count           = số bản ghi thực tế lấy được trong toàn khung theo dõi
...
└─ count >= frameNum (và count > 0)
      ...
      │     ├─ đúng vào khung giờ kết thúc theo dõi → MOTION_SENSOR (101) "có người"
      │     └─ khung khác                            → NOT_APPLICABLE (100) (chặn báo giữa giờ)
      │
      ├─ (không rơi vào nhánh trên) có ≥1 bản ghi c006 < 0 (giá trị âm — dấu hiệu lỗi cảm biến/bù vòng)
...
**⚠️ Điểm bất thường của hệ cũ**: biến `$resultCode` trong `checkMonitoring2` được gán `0` làm giá trị
"chưa xác định" rồi so sánh `if ($resultCode != 0)` — nhưng `0` **không phải** 1 trong 5 hằng số trạng
thái hợp lệ (100-104), chỉ là sentinel nội bộ. Nếu vòng `foreach` gặp nhiều bản ghi thỏa điều kiện với
kết quả khác nhau (1 bản ghi ở giữa giờ, 1 bản ghi ở cuối giờ), **bản ghi xét sau cùng trong vòng lặp
thắng** — thứ tự lặp phụ thuộc thứ tự SQL trả về, không có `ORDER BY` rõ ràng trong `getTargetData2`
cho nhánh này — kết quả có thể không ổn định giữa các lần chạy với dữ liệu giống nhau.
...
- 6 mẫu thông báo cố định (`ConRegularMessages`, id `'1'`–`'6'`) — **nội dung tiêu đề/message thật
  ...
  | `6` | `MISSING_DATA` (chỉ từ 見守り②) | *(suy đoán)* "mất dữ liệu liên tục" |
...
- Lỗi khi lưu (`saveOrFail`) → bắt exception, ghi log thường (không phải `alert`), set `$resultCode =
  false` — **không rollback dữ liệu đã lưu trước đó trong vòng lặp** (không có transaction bọc ngoài).
  (`WatchNotificationCommand.php:282-291`)
- Sau khi lưu thành công → update `t_901.c006` (`setBeforeNoticed`) + `c007` (`setModified`) cho từng
  hộ trong danh sách đã xử lý ở bước ghi kết quả — lỗi update thì ghi log `alert` (khác lỗi ghi
  Push/Message ở trên, mức nghiêm trọng cao hơn). (`WatchNotificationCommand.php:307-333`)
...
- Phụ thuộc trực tiếp vào `CalcTenMinutesSensorCommand` chạy trước (lệch 5 phút) — nếu batch đó lỗi/trễ,
  ...
  - 見守り② còn phụ thuộc gián tiếp `getContinuousMissingData` (§A.2.4) chạy trước và có độ ưu tiên cao
    hơn — 1 hộ có thể được báo "mất dữ liệu liên tục" dù thực ra chỉ là 見守り② đang ở nhánh cảnh báo
    dữ liệu âm (bug §A.2.6), gây trùng lặp ý nghĩa giữa 2 cơ chế báo "mất dữ liệu" trong cùng 1 batch.
...
|---|---|---|---|---|
| Nhận sự kiện cảm biến IoT + xác định hộ cần push | `src/functions/batch-receive-data-infrared-remote/app.ts` | `BatchReceiveDataInfraredRemoteStateMachine` (`src/statemachine/batch-receive-data-infrared-remote.asl.json`) — **trigger real-time bởi event cảm biến IoT** (payload `event: SENSOR_AUTO_REPORT`/`EXECUTE_AUTOMATION`), không phải cron | Event trực tiếp từ cảm biến IoT của MUI (`payload.motion`, `payload.wbgt`, `payload.temperature`) | Ghi lịch sử `TABLE_INFRARED_REMOTE_DATA`; đẩy danh sách hộ cần push qua S3 tạm (`createDataSegment`) |
| Gửi push "room monitor" (+ điều khiển thiết bị nếu có lịch) | `src/functions/batch-control-device-and-push-notice-sensor/app.ts` | Bước `Map` (DISTRIBUTED) ngay sau bước trên trong cùng state machine | Đọc lại segment từ S3, đọc `TABLE_USER_SETTING` (cờ `flag_push_notice_room_monitor`) | `pushNotificationFirebase()` — push trực tiếp tới thiết bị di động qua Firebase, không ghi bảng "message"/app-inbox tương đương `ConMessage` |
...
| Trạng thái "mất dữ liệu liên tục" | Có, 2 nguồn (§A.2.4 + bug §A.2.6, id message 6) | **Không tìm thấy** |
...
- **見守り②** — xét **nhiều khung liên tiếp** (tối thiểu 3 khung = 30 phút) trước khi kết luận, để tránh
  báo giả từ 1 lần đo hụt. Hợp tình huống theo dõi liên tục kéo dài (vd. theo dõi người già cả buổi).
...
- Bỏ hẳn: khung giờ theo dõi (start/end), ngưỡng tối thiểu nhiều khung của ②, và cả 3 trạng thái "không
  có người"/"không có dữ liệu"/"mất dữ liệu liên tục".
