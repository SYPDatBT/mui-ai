...
thiết bị `t_301` giữ 13 tháng, lịch sử phân khúc nhóm khách hàng `s_151` giữ 24 tháng, điểm tiết kiệm
năng lượng `s_141` giữ 2 năm tài chính). Ở repo mới `syp-eminelstandard-backend`, phần (1) đã có kết luận ở `CreateTablePartition.md`
...
| Mục | Nội dung |
|---|---|
| **Tên batch** | Class: `DeleteDataCommand` (extends `Command` — **không** extends `BaseCommand`, khác với batch cặp đôi `CreateTablePartitionCommand`, xem A.2.4) · Tên lệnh gọi thực tế trong wrapper: `DeleteData` (`cake.php DeleteData` — `12_CreateCsvAndDeleteData_day1.sh:32`, `_day2to31.sh:26`) · Script cron: `12_CreateCsvAndDeleteData_day1.sh` (ngày 1 đầu tháng) / `12_CreateCsvAndDeleteData_day2to31.sh` (các ngày còn lại) · Tên tiếng Nhật trong cron: "12.DBデータ削除". |
...
| Mục | Nội dung | Nguồn |
|---|---|---|
| Lịch chạy (cron) | `15 5 * * *` (`_day2to31.sh` — chạy MỌI ngày kể cả ngày 1: biểu thức cron KHÔNG có trường day-of-month, tên script không phản ánh cron thật) và `15 5 1 * *` (`_day1.sh` — chỉ ngày 1) — cùng 05:15, nên ngày 1 CẢ 2 script chạy chồng nhau (`flock` khóa theo `$0` từng file, không chặn chéo) — quirk vận hành thật của hệ cũ, vô hại về kết quả vì DROP có kiểm tra tồn tại và DELETE theo điều kiện cố định; ngày 1 chạy 4 lệnh `CreateCsvAndZip*` (day2to31 chỉ 2 lệnh) — đúng là có thêm bước xuất CSV | `docs/02_詳細設計/10_バッチ処理/mng-webap_cron設定_20241029.txt:39-41`; nội dung `.sh` trong `cron実行用シェルスクリプト/eminel-mng-webap.20240909.tgz` |
...
**Script cron `CreateCsvAndDeleteData` gọi CHUNG với các Command xuất CSV** *(đã xác nhận trực tiếp
từ nội dung `.sh` trong tgz)*: không có cron entry riêng nào cho các Command `CreateCsvAndZip*` ở
CẢ 2 file cron (`webap_cron設定_*.txt`, `mng-webap_cron設定_*.txt` — đã grep, 0 kết quả) — các Command
đó được gọi từ trong chính wrapper `12_CreateCsvAndDeleteData_*.sh` này, theo thứ tự `CreateCsvAndZip*`
(4 lệnh ở day1 / 2 lệnh ở day2to31) → `DeleteData` → `DeleteLogicalDeletedDevices` (batch số 4 cùng
nhóm), với `set -eu`: CSV fail thì lệnh xóa KHÔNG chạy — thứ tự CSV-trước-delete là chủ ý thiết kế
(「CSV作成が失敗した場合はDBのデータを消去するコマンドは実施されない。」 — `cron設定概要.txt:30-32`).
Khớp với mapping đã xác nhận ở `README.md:159`: mục docs "08_データ削除と過去データCSV作成" ↔ CHÍNH XÁC
2 nhóm Command này (`DeleteDataCommand` + `CreateCsvAndZip*Command`).
...
|---|---|---|---|---|
| `deleteConDeviceControls` | `ConDeviceControls` (bảng `t_301`) | デバイス制御履歴 — lịch sử lệnh điều khiển thiết bị | `created < (đầu tháng hiện tại − 13 tháng)` | `:126-141`, cột `C_CREATED='c004'` (`Entity/ConDeviceControl.php:30`), `setTable('t_301')` (`Table/ConDeviceControlsTable.php:41`) |
...
| `deleteConEcoPoints` | `ConEcoPoints` (bảng `s_141`) | 省エネポイント — điểm tiết kiệm năng lượng | `year <= (năm tài chính hiện tại − ECO_POINTS_SAVE_TIME)` | `:201-224`, hằng số `ECO_POINTS_SAVE_TIME=2` (`const.php:723`), `setTable('s_141')` (`Table/ConEcoPointsTable.php:41`) |
...
- **Không có try/catch ở bất kỳ bước xóa nào** (khác hẳn `CreateTablePartitionCommand` — mỗi `CREATE
  ...
  xóa này, TOÀN BỘ 13 bước gọi liên tiếp trong `execute()` (`:47-65`) không có bất kỳ xử lý lỗi nào — 1
...
- **Không extends `BaseCommand`** — khác với `CreateTablePartitionCommand` (và 18 Command khác), batch
  này KHÔNG có cơ chế lock PID chống chạy trùng ở tầng Command; tầng wrapper `.sh` thì CÓ `flock -n`
  chống chạy trùng chính nó (dòng 6 mỗi script — mục đích 重複起動排除, `cron設定概要.txt:3-4`).
  *(suy đoán: có thể vì mọi phép xóa ở đây đều tự nhiên an toàn khi chạy trùng — `DROP TABLE` có kiểm
  tra tồn tại trước, `deleteAll` theo điều kiện thời gian cố định trong ngày không đổi giữa 2 lần chạy
  gần nhau — nên Command không cần lock riêng; nhưng đây chỉ là suy đoán về lý do, không xác nhận được
  chủ ý thật của tác giả).*
...
- Việc DROP hẳn 1 bảng partition (thay vì archive) là KHÔNG HỒI PHỤC được nếu không có bước export CSV
  trước đó thành công — rủi ro này đã được thiết kế chặn ở tầng wrapper (xem A.2.1): `.sh` chạy CSV
  TRƯỚC delete với `set -eu`, CSV fail thì `DeleteData` không được gọi (`cron設定概要.txt:30-32`).
...
> bảng DELETE-theo-điều-kiện ở A.2.3 (`ConDeviceControls`=`t_301`, `ConUserGroupHistories`=`s_151`,
> `ConEcoPoints`=`s_141`).
...
- 2/3 khái niệm (`ConDeviceControls`=`t_301`, `ConUserGroupHistories`=`s_151`) chưa có dữ liệu tương
  ứng ở hệ thống mới hiện tại — không phải "thiếu cơ chế xóa", mà là bản thân TÍNH NĂNG lưu trữ dữ liệu
  đó chưa tồn tại (lịch sử điều khiển thiết bị chỉ xử lý real-time, không persist; tính năng so sánh
  nhóm chưa port — đã xác nhận ở `CreateGroupSummary.md`). Riêng グループ履歴 (`s_151`): tính năng so
  sánh nhóm đã CHỐT 「実施する」 (C2 ランキング — mọi 平均/ランキング phụ thuộc グルーピング batch
  月1回) nên dữ liệu này SẼ xuất hiện ở hệ mới — yêu cầu giữ 24 tháng phải được mang sang bàn thiết kế
  グルーピング mới, không thể xếp vào diện "không cần lo retention".
...
| Hệ thống cũ | Nội dung wrapper `.sh` (thứ tự CSV→delete, `flock`, `set -eu`) + chủ ý thiết kế | `docs/02_詳細設計/10_バッチ処理/cron実行用シェルスクリプト/eminel-mng-webap.20240909.tgz` (`12_CreateCsvAndDeleteData_day1.sh`, `_day2to31.sh`), `cron設定概要.txt:3-4,30-32` |
