...
"điều khiển thiết bị từ xa" (`instruction_type=1` — do user bật/tắt thiết bị từ xa qua app, hoặc do
batch DR `ControlDrOperationCommand` ghi vào) đã nằm trong hàng đợi `Instructions` quá 4 phút mà GW
(gateway tại nhà) chưa lấy/xử lý xong — đây là safety-net cho mô hình **polling-based control**: bình
thường lệnh tự bị xóa khi GW báo hoàn tất (`InstructionController.php:548-563`), batch này chỉ dọn phần
"lệnh chết" khi GW mất kết nối/không phản hồi kịp (kể cả lệnh DR bị timeout), tránh hàng đợi phình to vô
hạn. Ở repo mới `syp-eminelstandard-backend`, **không cần và không có cơ chế tương đương** — không phải
vì thiếu sót khi port, mà vì kiến trúc điều khiển thiết bị đã đổi hẳn về chất: thay vì ghi lệnh vào hàng
đợi cho GW polling lấy về, hệ thống mới gọi THẲNG API cloud của hãng thiết bị (Rinnai/Noritz/Daikin) và
chờ response ĐỒNG BỘ ngay trong cùng 1 lần chạy Lambda — không còn khái niệm "lệnh đang chờ xử lý" tồn
tại lâu dài trong DB, nên không có gì để timeout/dọn rác.
...
| Mục | Nội dung |
|---|---|
| **Tên batch** | Class: `DeleteTimeOutControlOneMinuteCommand` (extends `Command` trực tiếp) · Tên lệnh gọi: `DeleteTimeOutControlOneMinute` (`12_DeleteTimeOutControlOneMinute.sh:20` — `bin/cake.php DeleteTimeOutControlOneMinute`) · Script cron: `12_DeleteTimeOutControlOneMinute.sh` · Cùng nhóm cron "12.DBデータ削除" với `DeleteDataCommand`/`DeleteTimeOutControlTenMinuteCommand`. |
...
1. Lệnh `instruction_type=1` được ghi vào `Instructions` từ 2 nguồn: (a) user ra lệnh điều khiển thiết
   bị từ xa qua app → server ghi 1 dòng (`SetDataController.php:384,413`); (b) batch DR hệ cũ cũng ghi
   lệnh loại 1 (`ControlDrOperationCommand.php:215`, comment 「宅外制御指示」) — nên batch này dọn cả
   lệnh DR bị timeout. (khóa `ems_sp_no`+`node_id`+`eoj`, kèm `instruction_content`).
...
- Không extends `BaseCommand` → bản thân PHP Command không có lock PID chống chạy trùng — nhưng script
  cron `12_DeleteTimeOutControlOneMinute.sh:5-9` đã chặn sẵn ở tầng shell bằng `flock -n`
  (「多重起動チェック」: đang có tiến trình chạy → in `Already running.` rồi thoát), nên khi chạy qua
  script cron chuẩn không xảy ra 2 tiến trình xử lý chồng lấn kể cả khi 1 lượt chạy mất hơn 60 giây.
  (script nằm trong `docs/02_詳細設計/10_バッチ処理/cron実行用シェルスクリプト/eminel-mng-webap.20240909.tgz`)
...
|---|---|
| Mô hình điều khiển thiết bị thật | **PUSH đồng bộ, không polling**: `src/functions/api-device/control-device.ts:91-425` (user bấm điều khiển), `src/functions/batch-execute-automation/app.ts:153-159` (lịch tự động) — và các đường điều khiển còn lại ở dòng dưới — gọi THẲNG API cloud hãng (`rinnaiService`/`noritzService`/`daikinService`) qua `src/layers/common/nodejs/business-logic/control-device.ts:226-793`, `await` kết quả ngay trong cùng lần chạy Lambda — không ghi "lệnh đang chờ" vào bảng nào cả. |
| Các đường điều khiển còn lại (kiểm bổ sung) | Cùng mô hình đồng bộ: DR gọi thẳng `controlDevice` (`batch-start-dr/app.ts:110,163,191`, `DEVICE_LASTEST_CONTROL_BY.DR`) hoặc gọi thẳng service hãng (`batch-end-dr/app.ts:206-330`); điều khiển theo sensor (`batch-control-device-and-push-notice-sensor/app.ts:153-225`) cũng gọi thẳng `rinnaiService`/`noritzService`; thiết bị mui M300 đi qua chính `control-device.ts` (`SERVER_TYPE.MUI_CLOUD` `:199` → `controlMuiDevice` `:363,452,470`, định nghĩa `:867`) gọi `infraredRemoteService` (axios, `services/infrared-remote.ts`) — không đường nào ghi hàng đợi. Statemachine `batch-control-device-by-temperature.asl.json` chỉ còn định nghĩa ASL, Lambda tương ứng không có trong `src/functions/` — càng củng cố kết luận. |
...
| Hệ thống cũ | Nguồn ghi lệnh `instruction_type=1` (app / DR) | `sources/conciergesv-develop/src/Controller/SetDataController.php:384,413` ・ `src/Command/ControlDrOperationCommand.php:210-215,288` |
...
| Hệ thống cũ | Script cron (flock 多重起動チェック + tên lệnh gọi) | `docs/02_詳細設計/10_バッチ処理/cron実行用シェルスクリプト/eminel-mng-webap.20240909.tgz` → `eminel-mng-webap/12_DeleteTimeOutControlOneMinute.sh:5-9,20` |
...
| Hệ thống mới | Các đường điều khiển còn lại (DR / sensor / mui M300) | `src/functions/batch-start-dr/app.ts:110,163,191` ・ `src/functions/batch-end-dr/app.ts:206-330` ・ `src/functions/batch-control-device-and-push-notice-sensor/app.ts:153-225` ・ `src/layers/common/nodejs/business-logic/control-device.ts:199,363,452,470,867` |
