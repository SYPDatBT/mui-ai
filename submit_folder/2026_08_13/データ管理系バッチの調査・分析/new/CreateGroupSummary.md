...
mỗi nhóm — 1 device_type chính thức, không phải hack), đồng thời lưu lại **lịch sử phân khúc của từng
...
theo tổ hợp nhóm/`device_type` đặc biệt (đã soát toàn bộ 53 bảng trong `template-dynamodb.yaml`),
...
| Mục | Nội dung |
|---|---|
| **Tên batch** | Class: `CreateGroupSummaryCommand` (extends `BaseCommand`) · Tên lệnh gọi: `CreateGroupSummary` *(theo shell script cron thật trong tgz: `sudo -u apache php /var/www/vhost/conciergesv/bin/cake.php CreateGroupSummary` — CakePHP resolve được cả dạng CamelCase lẫn snake_case)* · Script cron: `14_CreateGroupSummary.sh` · Tên tiếng Nhật trong cron: "14.グループ集計情報登録機能" (chức năng đăng ký thông tin tổng hợp nhóm). |
...
- Mục đích cuối của bảng `s_151`, xác nhận qua doc comment entity: "アプリのグループ平均表示のための情報。
  ...
  2 bảng `s_113`/`s_114`: `GetUsageController.php` (API lấy usage cho app) có 2 nhánh JOIN riêng — nhánh
  so sánh THEO NĂM join `ConUserGroupHistories` (`s_151`) với `ConSensorMonthlyAveValues` (`s_114`);
  nhánh so sánh THEO NGÀY/THÁNG **không dùng `s_151`** mà join `ConSensorDailyValues` (`s_103` — bảng
  sensor theo tháng per-user, đã được batch khác đóng dấu sẵn `group_attr1..5` vào từng dòng;
  `ConSensorDailyValuesTable.php:41`) với `ConSensorDailyAveValues` (`s_113`). Cả 2 nhánh đều lọc
  `device_type=16` + cùng tổ hợp `group_attr` (bucket hóa LẠI ngay trong câu JOIN, y hệt logic A.2.2) để
  lấy giá trị trung bình nhóm hiển thị cho khách hàng. Lưu ý cho thiết kế port (Q-G6-1 về 平均 liên hộ
  của `s_113`): luồng daily phụ thuộc việc `group_attr` được ghi vào từng dòng `s_103`, không chỉ phụ
  thuộc `s_151`. (`GetUsageController.php:356-442,745-826`)
...
- **Cơ chế chống chạy trùng ở tầng ứng dụng** (không riêng batch này) — `BaseCommand` (class cha) tạo file
  ...
  còn sống thì instance mới **exit ngay, không log lỗi rõ ràng** (riêng batch này, shell script cron
  `14_CreateGroupSummary.sh` còn có `flock` ở tầng shell → chống trùng kép shell + application). Cơ chế
  lock này dùng chung bởi 18 Command khác trong `conciergesv` (19 Command tổng cộng kể cả batch này, đếm
  thật qua grep `extends BaseCommand`), không phải riêng cho batch này — gồm cả nhóm `Calc*Command`
  (10 phút/ngày/tháng/năm), `RankingCreationCommand`, `WatchNotificationCommand`,
  `ControlDrOperationCommand`,... (`BaseCommand.php:21-38`)
...
| Toàn bộ `template-dynamodb.yaml` (53 bảng) | Không có bảng nào tên chứa "Group"/"Segment"/"Population"; các bảng usage (`DeviceDailyUsageHistoryTable`, `DeviceMonthlyUsageHistoryTable`) có PK = `receive_date`+`history_id`, truy vấn per-user qua GSI `gsi_tagtag_kaiin_bango` — không có key/cột nào theo tổ hợp 5 thuộc tính hay `device_type` đặc biệt như `s_113`/`s_114`. |
...
| Hệ thống cũ | Nơi đọc lại `s_113`/`s_114`/`s_151` (+`s_103`, consumer, xác nhận mục đích) | `sources/conciergesv-develop/src/Controller/GetUsageController.php:356-442,745-826` |
