...
| Mục | Nội dung |
|---|---|
| **Tên batch** | Class: `RankingCreationCommand` (extends `BaseCommand`) · Lời gọi thật trong cron script: `RankingCreation` (PascalCase, theo đúng quy ước các Command khác); riêng docblock ghi `rankingCreation` (lowerCamelCase) — lệch nhỏ trong tài liệu hóa của chính file này · Script cron: `20_RankingCreation.sh` · Tên tiếng Nhật trong cron: "20.ランキング作成". |
...
Với mỗi khách hàng, mỗi loại ranking (1 CO2 tổng / 2 CO2 gas / 3 CO2 điện):
  ...
  TRONG nhóm 5-thuộc-tính   TRONG nhóm rộng hơn (chỉ loại nhà + nguồn nhiệt sưởi (暖房熱源), bỏ 3 thuộc tính còn lại)
...
| `--rankingtype` | **Không có default** — bắt buộc phải truyền, comma-separated, mỗi giá trị phải là `1`(tổng)/`2`(gas)/`3`(điện), sai → abort ngay. Xác nhận trực tiếp từ `20_RankingCreation.sh` (trong `eminel-mng-webap.20240909.tgz`): cron gọi 1 lần với cả 3 loại `--rankingtype=1,2,3`, không truyền `--yearmonth` (→ mặc định tháng trước). | `RankingCreationCommand.php:69-82` |
...
| `s_114` (`parts_attr`, fallback) | Tổng population các nhóm 5-thuộc-tính có CÙNG 2 thuộc tính đầu (loại nhà + nguồn nhiệt sưởi (暖房熱源)), dùng khi nhóm 5-thuộc-tính ≤9 người | `RankingCreationCommand.php:220-233` |
...
| Grep `CalcCarbonDioxide`/`ConRanking` trên toàn repo | 0 kết quả trong code. |
...
| Hệ thống cũ | Nhóm 5-thuộc-tính & population (dùng chung logic) | `CreateGroupSummary.md` (cùng thư mục) |
