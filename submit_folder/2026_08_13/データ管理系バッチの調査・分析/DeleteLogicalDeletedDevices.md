...
`DeleteLogicalDeletedDevicesCommand` là **CLI chạy thủ công** (không có lịch cron) trong `conciergesv`
của hệ thống cũ, nhưng thao tác lên các bảng gốc thuộc domain HEMS dùng chung cả 3 server
(`Devices`/`DeviceDatas`/`ErrorDeviceDatas`/`Instructions`): tìm các thiết bị đã bị **soft-delete**
(`delete_flg=true` — do API `NotifyDeviceListController` ở `hemssv` đặt khi GW báo thiết bị không còn
tồn tại) và đã soft-delete quá 30 ngày, rồi **hard-delete** (xóa vật lý) toàn bộ dữ liệu đo/lỗi/lệnh
điều khiển liên quan + chính record thiết bị đó, trong 1 transaction. Đây là bước "dọn dẹp thật" sau
grace period 30 ngày của vòng đời gỡ thiết bị IoT. Ở repo mới `syp-eminelstandard-backend`, **không tìm
thấy cơ chế tương đương** — cả 2 vế: (1) không có cờ soft-delete nào trên bảng thiết bị (`DeviceTable`,
`MuiDeviceTable`) tương đương `delete_flg`, không có API nào tương đương `NotifyDeviceListController`
để tự động đánh dấu thiết bị mất kết nối; (2) xóa thiết bị ở hệ mới (`api-device/delete-sensor.ts`) là
hard-delete NGAY khi người dùng chủ động gọi API, không qua bước soft-delete + grace period 30 ngày nào,
và không có TTL/batch nào dọn "thiết bị ma" theo thời gian. Đây không phải thiếu 1 batch dọn dẹp, mà là
bản thân khái niệm nghiệp vụ "soft-delete thiết bị khi GW báo mất, chờ 30 ngày rồi xóa hẳn" chưa tồn tại
ở kiến trúc mới.
...
| Mục | Nội dung |
|---|---|
| **Tên batch** | Class: `DeleteLogicalDeletedDevicesCommand` (extends `Command` trực tiếp, không `BaseCommand`) · Tên lệnh gọi: `delete_logical_deleted_devices` *(suy đoán theo quy ước CakePHP 4)* · **Không có lịch chạy — CLI chạy tay** (đã grep cả 2 file cron `webap_cron設定_20240905.txt` và `mng-webap_cron設定_20241029.txt`, không có kết quả). |
...
| Phạm vi dùng chung | Cả 4 bảng (`Devices`,`DeviceDatas`,`ErrorDeviceDatas`,`Instructions`) được đọc/ghi bởi 26 file khác trên CẢ 3 server (`hemssv`,`eminelsv`,`conciergesv` — đếm thật qua grep) — xác nhận đây là dữ liệu lõi domain HEMS dùng chung toàn hệ thống, không riêng gì `conciergesv`. | grep `EminelSvLib.(Devices\|DeviceDatas\|ErrorDeviceDatas\|Instructions)` trên `sources/` |
...
- **Batch chạy tay, không có lịch cron** — khác hẳn các batch dọn dẹp khác đã audit (`DeleteDataCommand`
  chạy cron hàng ngày). Nghĩa là việc dọn "thiết bị ma" phụ thuộc hoàn toàn vào việc có người vận hành
  chủ động chạy lệnh này định kỳ — không tự động. Không có tài liệu nào (cron hay docs đã đọc) xác nhận
  quy trình vận hành thật chạy batch này theo chu kỳ nào.
...
- Không extends `BaseCommand` → không có cơ chế lock PID chống chạy trùng (giống nhận xét ở
  `DeleteData.md`) — nhưng batch này còn rủi ro hơn vì có thể chạy tay bất kỳ lúc nào (không theo lịch
  cố định), nên khả năng 2 người/2 tiến trình cùng chạy trùng thời điểm khó kiểm soát hơn 1 batch cron.
...
> tra và lý do không khớp (thay cho B.1/B.2).
...
| Grep `NotifyDeviceList`/`delete_flg` trên toàn `src/` | 0 kết quả — không có API nào tương đương `NotifyDeviceListController` (nơi phát sinh soft-delete ở bản cũ). |
| `src/functions/api-device/delete-sensor.ts:54-121` | Candidate gần giống nhất về mặt kỹ thuật (xóa `MuiSensor` + `MuiDevice` con trong 1 transaction), nhưng khác bản chất: (1) do NGƯỜI DÙNG chủ động gọi API xóa ngay, không phải batch chạy theo điều kiện thời gian; (2) không qua bước soft-delete + grace period 30 ngày nào; (3) không xóa dữ liệu đo/lỗi/lệnh chờ tương đương `DeviceDatas`/`ErrorDeviceDatas`/`Instructions` (dữ liệu đo ở hệ mới dùng bảng khác đã có TTL sẵn — `DeviceMonthlyUsageHistoryTable`,... — không cần dọn thủ công theo thiết bị). |
...
| ~70 thư mục `batch-*` trong `src/functions/` | Không có tên nào liên quan "delete-device"/"hard-delete"/"device-cleanup"/"purge-device"/"delete-logical". |

---

## Tổng kết

Không có — bản cũ chỉ có 1 luồng xử lý đơn giản (không nhánh/thuật toán song song), và hệ thống mới
**không tìm thấy gì** để đối chiếu (không phải trường hợp "thay bằng cơ chế khác về chất") — bảng "Đã
kiểm tra" ở Phần B đã nêu đủ lý do không khớp cho từng candidate. Đáng chú ý: đây không phải 1 tính năng
bị "bỏ sót" khi port, mà là khái niệm nghiệp vụ gốc (soft-delete thiết bị khi GW báo mất kết nối, giữ 30
ngày grace period rồi mới xóa hẳn) chưa tồn tại ở kiến trúc mới — tương tự dạng gap đã ghi nhận ở
`DeleteData.md` cho `ConEcoPoints` (có dữ liệu nhưng thiếu chính sách vòng đời, khác ở đây là còn chưa
có cả khái niệm dữ liệu).
...
| Hệ thống cũ | Phạm vi dùng chung 3 server (đếm thật) | grep `EminelSvLib.(Devices\|DeviceDatas\|ErrorDeviceDatas\|Instructions)` trên `sources/` — 26 file |
...
| Hệ thống mới | API xóa thiết bị thật (khác bản chất — hard-delete ngay, không qua soft-delete) | `src/functions/api-device/delete-sensor.ts:54-121` |
