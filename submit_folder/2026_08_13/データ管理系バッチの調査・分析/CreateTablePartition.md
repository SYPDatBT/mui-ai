...
28 Command khác (đếm thật qua grep) ghi dữ liệu vào các bảng đó không bị lỗi insert. Ở repo mới
...
| `rankingInfo` | `s_121` (`ConRanking`) | ランキング情報 | Năm | Có | `[năm, năm+1)` — rộng cả năm | Dữ liệu ranking phát sinh vào các ngày/tháng khác nhau trong năm, không cố định 1 giá trị |
| `deviceStatus` | `t_202` (`ConDeviceStatus`) | 機器状態情報 — trạng thái thiết bị (ghi qua API `GetDataController`, không chỉ batch) | Ngày | Không | `[ngày, ngày+1)` | Dữ liệu trạng thái ghi liên tục theo thời gian thật nhận từ thiết bị |
...
- **Đây là batch hạ tầng nền cho rất nhiều batch khác** — grep `EminelSvLib.ConSensor*`/`ConWeeklyEcoReports`/
  `ConRankings` trong `src/Command/` của `conciergesv` ra **28 file Command khác** thực sự ghi vào 1
  trong 10 bảng này (nhóm `Calc*Command`, `RankingCreationCommand`, `CreateGroupSummaryCommand`,
  `DistributeMonthlyEcoPointsCommand`, các `PublishRegularEcoMission\*Publisher`,...); riêng `t_202`
  (trạng thái thiết bị) còn được ghi qua **API** `GetDataController` (không phải batch) — nghĩa là API
  nhận dữ liệu thiết bị theo thời gian thực CŨNG phụ thuộc batch này đã tạo sẵn partition ngày hôm đó,
  không chỉ các batch cron.
...
- **Cơ chế chống chạy trùng ở tầng ứng dụng** (không riêng batch này, xem thêm `CreateGroupSummary.md`)
  — `BaseCommand` tạo file `.lock` theo PID, dùng chung bởi 19 Command khác trong `conciergesv`.
...
| Hệ thống cũ | Ý nghĩa cột ngày của từng bảng (hẹp vs rộng range) | `sources/eminel_sv_lib-develop/src/Model/Entity/{ConSensorHourlyValue,ConSensorDailyValue,ConSensorMonthlyValue,ConWeeklyEcoReport}.php` |
| Hệ thống cũ | 28 Command khác ghi vào các bảng này (cross-cutting, đếm thật) | grep `EminelSvLib.ConSensor*`/`ConWeeklyEcoReports`/`ConRankings` trong `sources/conciergesv-develop/src/Command/` |
| Hệ thống cũ | API cũng ghi `t_202` (ngoài batch) | `sources/conciergesv-develop/src/Controller/GetDataController.php` |
